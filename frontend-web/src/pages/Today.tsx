...
function DailyBriefCard() {
  ...
  async function fetchBrief() {
    ...
    try {
      ...
      setError(e?.response?.data?.error?.message ?? 'Failed to generate brief.');
    } finally {
      setLoading(false);
    }
  }
      ...
      <div className="mb-6 p-4 rounded-xl border border-indigo-200 bg-indigo-50">
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-sm text-indigo-600 mt-0.5">Get AI-powered priorities for today</p>
          </div>
          <button
            onClick={fetchBrief}
            disabled={loading}
  ...
  return (
    <div className="mb-6 rounded-xl border border-indigo-200 bg-indigo-50 p-4 space-y-3">
      <div className="flex items-center justify-between">
        <p className="font-semibold text-indigo-900">🤖 Daily Strategy Brief</p>
        <button onClick={fetchBrief} disabled={loading} className="text-xs text-indigo-500 hover:text-indigo-700 disabled:opacity-40">
      ...
      <ol className="space-y-1.5">
        {brief.top_actions.map(a => (
          <li key={a.priority} className="flex items-start gap-2 text-sm">
            <span className="font-bold text-indigo-600 shrink-0">{a.priority}.</span>
      ...
      <div className="grid grid-cols-2 gap-2">
        {brief.finance_alert && (
          <div className="rounded-lg bg-white border border-blue-200 p-2">
            ...
            <p className="text-xs text-gray-600 mt-0.5">{brief.finance_alert}</p>
          </div>
        )}
        {brief.health_nudge && (
          <div className="rounded-lg bg-white border border-emerald-200 p-2">
            <p className="text-xs font-semibold text-emerald-700">🌿 Wellbeing</p>
