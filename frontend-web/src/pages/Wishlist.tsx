...
export default function Wishlist() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      setError(
        err?.response?.data?.error?.message ?? "Failed to load wishlist data.",
      );
    } finally {
      setLoading(false);
    }
  ...
  async function handleEvaluate(itemId: string) {
    ...
    try {
      const res = await api.post<WishlistItem>(
        `/wishlist/${itemId}/evaluate`,
        {},
      );
    ...
    <div className="flex flex-col gap-8 max-w-3xl mx-auto py-8 px-4">
        ...
        >
          ...
          <div>
            ...
            <input
              ...
              onChange={(e) =>
                setForm((f) => ({ ...f, cooldown_days: e.target.value }))
              }
            />
          </div>
      ...
      {items.length === 0 ? (
        <p className="text-center text-gray-400 text-sm">
          Your wishlist is empty. Add an item above.
        </p>
      ) : (
                ...
                <div className="flex items-start justify-between gap-4">
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex items-center gap-2 mt-1 flex-wrap">
                      ...
                      {item.roi_score != null && (
                        <span className="text-xs text-gray-500">
                          ROI: {item.roi_score}/10
                        </span>
                      )}
                      {item.emotional_score != null && (
                        <span className="text-xs text-gray-500">
                          Emotional: {item.emotional_score}/10
                        </span>
                      )}
                    </div>
                  </div>
                  ...
                  <div className="flex flex-col items-end gap-2 shrink-0">
                    {vc && (
                      <span
                        className={`text-xs font-semibold px-2 py-1 rounded-full ${vc.bg} ${vc.color}`}
                      >
                        {vc.label}
                      </span>
                    )}
