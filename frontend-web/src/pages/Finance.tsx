...
export default function Finance() {
  const api = useApi();
  const [tab, setTab] = useState<"accounts" | "transactions" | "transfer">(
    "accounts",
  );
  ...
  async function createAccount() {
    ...
    try {
      await api.post("/accounts", {
        ...accForm,
        balance: parseFloat(accForm.balance as string) || 0,
      });
  ...
  function addSplitRow() {
    setTxForm((p) => ({
      ...p,
      splits: [...p.splits, { category: "", amount: "", percentage: "" }],
    }));
  }

  function updateSplit(i: number, field: keyof SplitRow, value: string) {
    setTxForm((p) => {
      const splits = p.splits.map((s, idx) =>
  ...
  function removeSplit(i: number) {
    setTxForm((p) => ({
      ...p,
      splits: p.splits.filter((_, idx) => idx !== i),
    }));
  }
  ...
  async function createTransfer() {
    ...
    try {
      await api.post("/transfers", {
        ...trForm,
        amount: parseFloat(trForm.amount as string),
      });
      setTrForm({
        from_account_id: "",
        to_account_id: "",
  ...
  const btnPrimary =
    "px-4 py-2 bg-indigo-600 text-white rounded-lg text-sm font-medium hover:bg-indigo-700 disabled:opacity-40 transition-colors";
  const btnGhost =
      ...
      {summary && (
        <div className="grid grid-cols-3 gap-4 mb-6">
          {[
            {
              label: "Total Balance",
              value: fmt(summary.total_balance, summary.currency || "MXN"),
              color: "indigo",
            },
            {
              label: "Month Income",
              value: fmt(summary.month_income, summary.currency || "MXN"),
              color: "green",
            },
            {
              label: "Month Expenses",
              value: fmt(summary.month_expenses, summary.currency || "MXN"),
              color: "red",
            },
              ...
              <div className="bg-white rounded-xl border border-gray-200 shadow-sm p-4 mb-6">
                ...
                <div className="grid grid-cols-2 gap-3">
                  ...
                  <select
                    ...
                    onChange={(e) =>
                      setAccForm((p) => ({ ...p, currency: e.target.value }))
                    }
                  >
              ...
              <ul className="space-y-3">
                {accounts.map((acc) => (
                  ...
                  >
                    ...
                    <p
                      className={`text-lg font-bold ${acc.balance >= 0 ? "text-green-600" : "text-red-600"}`}
                    >
              ...
              <div className="bg-white rounded-xl border border-gray-200 shadow-sm p-4 mb-6">
                ...
                <div className="grid grid-cols-2 gap-3">
                  <select
                    ...
                    onChange={(e) =>
                      setTxForm((p) => ({ ...p, account_id: e.target.value }))
                    }
                  >
                  ...
                  <select
                    ...
                    onChange={(e) =>
                      setTxForm((p) => ({ ...p, currency: e.target.value }))
                    }
                  >
                    <option value="MXN">MXN</option>
                    <option value="USD">USD</option>
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      setTxForm((p) => ({ ...p, category: e.target.value }))
                    }
                  />
                  <input
                    className={inputCls}
                      ...
                      {txForm.splits.map((s, i) => (
                        <div key={i} className="flex gap-2 items-center">
                          ...
                          <input
                            ...
                            value={s.percentage}
                            onChange={(e) =>
                              updateSplit(i, "percentage", e.target.value)
                            }
                          />
                          <button
                ...
                <button
                  className={`mt-3 ${btnPrimary}`}
                  disabled={saving || !txForm.account_id || !txForm.amount}
                ...
                {transactions.map((tx) => (
                  ...
                  >
                    <div className="flex items-center gap-3">
                      <span
                        className={`text-lg ${tx.type === "income" ? "text-green-500" : "text-red-500"}`}
                      >
                        {tx.type === "income" ? "↑" : "↓"}
                      </span>
                      <div>
                        ...
                        {tx.splits && tx.splits.length > 0 && (
                          <p className="text-xs text-indigo-500">
                            {tx.splits.length} split
                            {tx.splits.length > 1 ? "s" : ""}
                          </p>
                        )}
                      </div>
                    </div>
          ...
          {tab === "transfer" && (
            <div className="bg-white rounded-xl border border-gray-200 shadow-sm p-4">
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                <select
                  className={inputCls}
                  value={trForm.to_account_id}
                  onChange={(e) =>
                    setTrForm((p) => ({ ...p, to_account_id: e.target.value }))
                  }
                >
                ...
                <input
                  ...
                  value={trForm.amount}
                  onChange={(e) =>
                    setTrForm((p) => ({ ...p, amount: e.target.value }))
                  }
                />
                <select
                  className={inputCls}
                  value={trForm.currency}
                  onChange={(e) =>
                    setTrForm((p) => ({ ...p, currency: e.target.value }))
                  }
                >
                  <option value="MXN">MXN</option>
                  <option value="USD">USD</option>
                ...
                <input
                  ...
                  value={trForm.date}
                  onChange={(e) =>
                    setTrForm((p) => ({ ...p, date: e.target.value }))
                  }
                />
                <input
                  ...
                  onChange={(e) =>
                    setTrForm((p) => ({ ...p, description: e.target.value }))
                  }
                />
              </div>
              <div className="flex gap-2 mt-4">
                <button
                  ...
                  disabled={
                    saving ||
                    !trForm.from_account_id ||
                    !trForm.to_account_id ||
                    !trForm.amount
                  }
